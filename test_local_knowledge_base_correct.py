...
async def test_knowledge_base():
    ...
    try:
        result = kb.query_database_knowledge.invoke({"query": "pizza"})
    ...
    # Test 2: Get available cuisines directly
    print("\n2️⃣ Testing get_available_cuisines:")
    try:
        result = kb.get_available_cuisines.invoke({})
        print(f"Result: {result}")
        ...
        # Check if Italian is mentioned
        if "Italian" in result:
            print("✅ Italian cuisine mentioned in available cuisines")
        else:
            print("❌ Italian cuisine NOT mentioned in available cuisines")
            
    except Exception as e:
        print(f"❌ Error: {e}")
    
    # Test 3: Get available restaurants
    print("\n3️⃣ Testing get_available_restaurants:")
    try:
        result = kb.get_available_restaurants.invoke({})
        print(f"Result: {result}")
